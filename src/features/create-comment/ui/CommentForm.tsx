  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        {!parentId && (
          <span className="shrink-0 inline-flex items-center gap-1 rounded-full bg-primary/8 border border-primary/20 px-2.5 py-1 font-mono text-xs font-semibold text-primary select-none">
        ...
        <Input
          ...
          placeholder={parentId ? '답글을 입력하세요...' : '이 시점에 피드백을 남겨보세요...'}
      ...
      {error && (
        <p className="text-xs text-destructive pl-2">{error}</p>
      )}
    </form>
  );
}
